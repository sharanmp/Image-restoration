...
def weiner_filter(blurred_image,kernel,K):
    fftType =0
    ...
    if fftType == 0:
        ...
        factor_B = np.divide(np.conjugate(fft_kernel_B), K + (abs(fft_kernel_B))**2)
        factor_G = np.divide(np.conjugate(fft_kernel_G), K + (abs(fft_kernel_G))**2)
        factor_R = np.divide(np.conjugate(fft_kernel_R), K + (abs(fft_kernel_R))**2)
...
def interactive_weiner_filter(blurred_image, kernel):
    K_init = 100
    K_min = 1
    K_max = 1000
    fig = plt.figure("Restored Image")
    deblurred_image = weiner_filter(blurred_image, kernel, K_init)
    ...
    sliderValue = Slider(axis, 'K', K_min, K_max, valinit=K_init)

    def update(gamma):
        updated_image = weiner_filter(blurred_image, kernel, gamma).astype(np.float32)
